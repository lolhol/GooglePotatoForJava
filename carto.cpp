...
CartoModule::CartoModule(const char *dir, const char *file, PoseUpdateCallback cb_function, bool use_imu, bool use_odom,
                         double lidar_scan_time_hz) : config_files_dir(dir),
                                                      config_file_name(file),
                                                      callback(cb_function),
                                                      using_imu(use_imu),
                                                      using_odom(use_odom),
                                                      radar_scan_time(from_hz_to_sec(lidar_scan_time_hz)) {
  // Set start time.
  ...
  trajectory_id = map_builder->AddTrajectoryBuilder(sensor_ids, trajectory_builder_options,
                                                    [this](const int id,
                                                           ...
                                                           const std::unique_ptr<
                                                               const ::cartographer::mapping::TrajectoryBuilderInterface::InsertionResult> res) {
...
int CartoModule::handle_lidar_data(PointCloudData &data) {
  ...
  if (latest_sensor_timestamp < 0 || latest_sensor_timestamp < data.timestamp) {
    trajectory_builder->AddSensorData(range_name, to_carto_point_cloud(data, radar_scan_time, startTime));
...
/*void CartoModule::paint_map_cur_visible(std::vector<char> *output) {
}*/
...
void CartoModule::OnLocalSlamResult2(
  ...
  Position pose2d = from_carto_rigid3d_raw(from_carto_time(time), pose3d);
